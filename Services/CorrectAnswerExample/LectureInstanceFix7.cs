...
namespace Services
{
    ...
    /// </summary>
    public class LectureInstanceFix7
    {
        ...
        public LectureInstanceFix7(IObserverConsole observer)
            : this(observer, 10, 2, 12, DateTime.Now.AddDays(10), AcademicLevel.None, AcademicLevel.Bachelor)
        { }

        public LectureInstanceFix7(IObserverConsole observer, int maxStudents, int maxLecturers, int maxSeats, DateTime lastTimetoRegister, AcademicLevel requiredToAttend, AcademicLevel requiredToTeach)
        {
        ...
        public bool AddAttendees(List<Person> p, bool isAdmin)
        {
            if (DateTime.Now < lastDayToRegister || isAdmin)
            {
                ...
                {
                    if (AddAttendee(person, isAdmin) == false)
                        return false;
                }
                return true;
            }
        ...
        private bool AddAttendee(Person p, bool isAdmin)
        {
            if (p is Student)
            {
                if (AddStudent(p, isAdmin) == false)
                    return false;
            }
            else if (p is Lecturer)
            {
                if (AddLecturer(p, isAdmin) == false)
                    return false;
            }
            else
            {
