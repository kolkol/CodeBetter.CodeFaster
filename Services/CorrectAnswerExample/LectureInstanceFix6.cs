...
namespace Services
{
    ...
    /// </summary>
    public class LectureInstanceFix6
    {
        ...
        public LectureInstanceFix6(IObserverConsole observer)
            : this(observer, 10, 2, 12, DateTime.Now.AddDays(10), AcademicLevel.None, AcademicLevel.Bachelor)
        { }

        public LectureInstanceFix6(IObserverConsole observer, int maxStudents, int maxLecturers, int maxSeats, DateTime lastTimetoRegister, AcademicLevel requiredToAttend, AcademicLevel requiredToTeach)
        {
        ...
        public bool AddAttendees(List<Person> p, bool isAdmin)
        {
            if (DateTime.Now < lastDayToRegister || isAdmin)
            {
                ...
                {
                    if (AddAttendee(p[i], isAdmin) == false)
                        return false;
                }
                return true;
            }
